...
export async function oauthRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  if (config.googleClientId && config.googleClientSecret) {
    ...
    fastify.log.warn('Google OAuth not configured — set GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET');
  }
  ...
  if (config.githubClientId && config.githubClientSecret) {
    ...
    fastify.log.warn('GitHub OAuth not configured — set GITHUB_CLIENT_ID + GITHUB_CLIENT_SECRET');
  }
}
